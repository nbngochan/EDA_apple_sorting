...
class ClassifierModel(LightningModule):
    ...
    def on_validation_epoch_end(self):
        ...
        df_cm = pd.DataFrame(confusion_matrix.cpu().data.numpy().astype(int), index=range(3), columns=range(3))
...
if __name__ == '__main__':
    ...
    data = SimpleData(
        ...
        class_list=['low', 'mid', 'top'],
    )
    model = ClassifierModel(
        model_name=args.model_name, pretrained=True,
        num_classes=len(data.classes), task='multiclass',
    )
